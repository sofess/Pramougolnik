...
class Rectangle
{
    ...
    public int Площадь() => Width * Height; // неправильно вычисляется площадь

    public static bool operator >(Rectangle a, Rectangle b) => a.Площадь() > b.Площадь(); //сравнение перепутано 
    ...
    {
        var p = str.Rectangle.Parse("Прямоугольник(10,20)"); //некорректная передача строки
